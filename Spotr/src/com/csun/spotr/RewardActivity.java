...
public class RewardActivity 
	...
	private GridView missingBadgeGridview;
	private BadgeAdapter badgeAdapter;
	private BadgeAdapter missingBadgeAdapter;
	private List<Badge> badgeList;
	private List<Badge> missingBadgeList;
	private GetBadgesTask gettask;
	private GetMissingBadgesTask getmissingtask;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		badgeGridview = (GridView) findViewById(R.id.badge_xml_gridview_userbadges);
		...
		missingBadgeGridview = (GridView) findViewById(R.id.badge_xml_gridview_missingbadges);
		missingBadgeAdapter = new BadgeAdapter(this, missingBadgeList);
		missingBadgeGridview.setAdapter(missingBadgeAdapter);
		missingBadgeGridview.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
				Intent intent = new Intent(getApplicationContext(), RewardViewActivity.class);
				intent.putExtra("id", badgeList.get(position).getId());
				intent.putExtra("name", badgeList.get(position).getName());
		...
		getmissingtask = new GetMissingBadgesTask(this);
		gettask = new GetBadgesTask(this);
		gettask.execute();
		getmissingtask.execute();
	}
		...
		protected Boolean doInBackground(Integer... offsets) {
			...
			if (array != null) {
				try {
					...
					Log.e(TAG + "GetMissingBadgesTask.doInBackGround(Integer... offsets) : ", "JSON error parsing data" + e.toString());
				}
				return true;
			}
			return false;
	...
	public void updateAsyncTask2Progress(Badge b) {
		...
		missingBadgeAdapter.notifyDataSetChanged();
	}
